...
- **Client onboarding order is fixed:** `/client/login` → `/client/onboarding` (Health Connect bottom sheet) → `/client/pair` → `/client` (home) → `/client/checkin`. Do not shortcut this chain.
