...
export const BODY_PARTS = [
  { id: "head",            label: "Head",             icon: "🧠", zMin: 2.2,  zMax: 3.2,  xMin: -0.5,  xMax: 0.5  },
  { id: "neck",            label: "Neck",             icon: "🔗", zMin: 1.6,  zMax: 2.2,  xMin: -0.35, xMax: 0.35 },
  { id: "left_shoulder",   label: "Left Shoulder",    icon: "💪", zMin: 0.6,  zMax: 1.6,  xMin: 0.6,   xMax: 1.42 },
  { id: "right_shoulder",  label: "Right Shoulder",   icon: "💪", zMin: 0.6,  zMax: 1.6,  xMin: -1.42, xMax: -0.6 },
  { id: "chest",           label: "Chest",            icon: "🫁", zMin: 0.2,  zMax: 1.6,  xMin: -0.6,  xMax: 0.6  },
  { id: "left_arm",        label: "Left Arm",         icon: "🦾", zMin: -1.5, zMax: 0.6,  xMin: 0.7,   xMax: 1.42 },
  { id: "right_arm",       label: "Right Arm",        icon: "🦾", zMin: -1.5, zMax: 0.6,  xMin: -1.42, xMax: -0.7 },
  { id: "abdomen",         label: "Abdomen",          icon: "🫃", zMin: -0.8, zMax: 0.2,  xMin: -0.7,  xMax: 0.7  },
  { id: "lower_back",      label: "Lower Back",       icon: "🦴", zMin: -1.8, zMax: -0.8, xMin: -0.7,  xMax: 0.7  },
  { id: "left_hip",        label: "Left Hip",         icon: "🦵", zMin: -2.5, zMax: -1.5, xMin: 0.1,   xMax: 0.9  },
  { id: "right_hip",       label: "Right Hip",        icon: "🦵", zMin: -2.5, zMax: -1.5, xMin: -0.9,  xMax: -0.1 },
  { id: "left_thigh",      label: "Left Thigh",       icon: "🦵", zMin: -3.8, zMax: -2.5, xMin: 0.0,   xMax: 0.85 },
  { id: "right_thigh",     label: "Right Thigh",      icon: "🦵", zMin: -3.8, zMax: -2.5, xMin: -0.85, xMax: 0.0  },
  { id: "left_knee",       label: "Left Knee",        icon: "🦿", zMin: -4.4, zMax: -3.8, xMin: 0.0,   xMax: 0.75 },
  { id: "right_knee",      label: "Right Knee",       icon: "🦿", zMin: -4.4, zMax: -3.8, xMin: -0.75, xMax: 0.0  },
  { id: "left_leg",        label: "Left Lower Leg",   icon: "🦵", zMin: -5.4, zMax: -4.4, xMin: 0.0,   xMax: 0.7  },
  { id: "right_leg",       label: "Right Lower Leg",  icon: "🦵", zMin: -5.4, zMax: -4.4, xMin: -0.7,  xMax: 0.0  },
  { id: "feet",            label: "Feet",             icon: "🦶", zMin: -5.9, zMax: -5.4, xMin: -0.9,  xMax: 0.9  },
];
...
const BodyViewer = forwardRef<BodyViewerHandle, BodyViewerProps>(
  function BodyViewer({ markedParts, background = "#0a0d14" }, ref) {
    ...
    useEffect(() => { markedRef.current = markedParts; }, [markedParts]);

    function getVertexIndices(part: typeof BODY_PARTS[0], positions: Float32Array) {
      const out: number[] = [];
      for (let i = 0; i < positions.length / 3; i++) {
        const x = positions[i * 3];
        const z = positions[i * 3 + 2];
        if (z >= part.zMin && z <= part.zMax && x >= part.xMin && x <= part.xMax) {
          out.push(i);
        }
      }
      return out;
    ...
    function repaint(marked: MarkedParts) {
      ...
      Object.entries(marked).forEach(([partId, type]) => {
        ...
        if (!part) return;
        const color = type === "pain" ? PAIN_COLOR : type === "active" ? ACT_COLOR : RECV_COLOR;
    ...
    useEffect(() => { repaint(markedParts); }, [markedParts]); // eslint-disable-line react-hooks/exhaustive-deps
